...
const App = () => {
  ...
  return (
    <div>
      {currentPage === 'login' && <Login onRegisterClick={handleRegisterClick} onLogin={handleLogin} registeredUsers={registeredUsers} />}
      {currentPage === 'register' && <Register onLoginClick={handleLoginClick} setRegisteredUsers={setRegisteredUsers} />}
      {isLoggedIn && <CustomAppBar onLogout={handleLogout} />}
    </div>
  );
};
