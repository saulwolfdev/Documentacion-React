  ...
  function MiComponente(props) {
    return (
        <div className="mi-componente-container">
          <h1 className="mi-componente-title">{props.titulo}</h1>
          <p className="mi-componente-description">{props.descripcion}</p>
        </div>
    );
  }
...
.mi-componente-description {
  ...
  dynamicVisibleFiles={["/App.js", "/MiComponente.js", "/styles.css"]}
/>
